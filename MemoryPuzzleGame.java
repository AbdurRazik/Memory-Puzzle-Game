...
public class MemoryPuzzleGame extends JFrame {
    ...
    private void startTimeAttackMode() {
        isTimeAttack = true;
        timeLeft = 60; // Reset time limit to 60 seconds for Time Attack
        if (countdownTimer != null) {
            countdownTimer.stop(); // Stop the timer if it was running
        }
        startGame();
    }
    ...
    private void startCountdownTimer() {
        if (countdownTimer != null) {
            countdownTimer.stop(); // Stop any previous timer
        }
